...
const Navbar: React.FC<NavbarProps> = ({ user, onLoginClick }) => {
  ...
  return (
    <>
      ...
      {/* Navbar */}
      <nav className="fixed top-0 left-0 right-0 z-50 w-full transition-all duration-300 bg-white shadow-s">
        <div
          className={`w-full transition-all duration-300 bg-white shadow-s px-4 sm:px-6 lg:px-8 py-4 ${isScrolled || !isHomePage
            ? "lg:bg-white lg:shadow-sm"
            : "lg:bg-white/20 lg:backdrop-blur-md lg:border-b lg:border-white/20 lg:shadow-none"
            }`}
        >
          <div className="flex justify-between items-center">
              ...
              {navLinks.map((link) => {
                ...
                return (
                  <Link
                    key={link.path}
                    to={link.path}
                    className={`transition font-medium relative group ${isScrolled || !isHomePage
                      ? active
                  ...
                  >
                    {link.label}
                    <span
                      className={`absolute -bottom-1 left-0 h-0.5 transition-all ${active ? "w-full" : "w-0 group-hover:w-full"
            ...
            {/* Mobile Menu Button - always solid/visible styling */}
            <div className="lg:hidden">
              <button
                onClick={() => setIsOpen(!isOpen)}
                className="p-2 rounded transition text-gray-700 hover:bg-gray-100"
              >
          ...
          {/* Mobile Menu - always solid/visible styling */}
          {isOpen && (
            <div className="lg:hidden mt-4 pt-4 animate-fadeIn border-t border-gray-200">
                ...
                {navLinks.map((link) => {
                  ...
                  return (
                    <Link
                      ...
                      className={`block px-4 py-3 rounded-lg font-medium transition ${active
