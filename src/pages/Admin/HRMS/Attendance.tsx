...
const TodayView = ({
  ...
  const handleSelectRecord = (empId: string) => {
  ...
  const handleSelectAll = () => {
    const pageIds = paginated.map(r => r.empId);
    const allSelected = pageIds.length > 0 && pageIds.every(id => selectedRecords.includes(id));
    if (allSelected) {
          ...
          <table className="hidden md:table min-w-full border-collapse text-[11px]">
            <thead className="sticky top-0 bg-slate-50 z-20 shadow-sm border-b border-slate-200">
              <tr className="bg-slate-50">
                <th className="px-3 py-2.5 w-8">
                  <input
                    type="checkbox"
                    checked={paginated.length > 0 && paginated.every(r => selectedRecords.includes(r.empId))}
            ...
            <tbody>
              ...
              {paginated.map((r, i) => {
                ...
                return (
                  <tr key={r.empId} className={`border-b border-slate-100 hover:bg-blue-50/20 transition duration-155 ${i % 2 ? "bg-slate-50/30" : ""} ${selectedRecords.includes(r.empId) ? "bg-[#0D47A1]/5" : ""}`}>
                    <td className="px-3 py-2.5 w-8">
                      <input
                        type="checkbox"
                        checked={selectedRecords.includes(r.empId)}
                        onChange={() => handleSelectRecord(r.empId)}
                        className="h-3.5 w-3.5 text-[#0D47A1] rounded border-slate-300 focus:ring-[#0D47A1] cursor-pointer"
                      />
                    </td>
