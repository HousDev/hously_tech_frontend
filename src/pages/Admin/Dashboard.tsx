...
const AdminDashboard = () => {
  ...
  const [pageSubtitleOverride, setPageSubtitleOverride] = useState('');

  // Map paths to default titles and subtitles
  ...
  useEffect(() => {
    const defaults = getHeaderFromPath(location.pathname);
    setHeaderTitle(defaults.title);
    setHeaderSubtitle(defaults.subtitle);
          ...
          <div className="flex-1 overflow-y-auto p-4">
            ...
            {activeMessageTab === 'alerts' && (
              <div className="space-y-3">
                ...
                {alerts.map((alert) => (
                  <div key={alert.id} className={`p-3 rounded-lg border ${alert.type === 'warning' ? 'border-yellow-200 bg-yellow-50' :
                    alert.type === 'success' ? 'border-green-200 bg-green-50' :
                      'border-red-200 bg-red-50'
                    }`}>
                    <div className="flex items-start">
                      <div className={`mr-3 p-2 rounded-full ${alert.type === 'warning' ? 'bg-yellow-100 text-yellow-600' :
        ...
        <main className="bg-[#f3f6fb] flex flex-col overflow-y-auto" style={{ height: 'calc(100vh - 64px)' }}>
          {isDashboard ? (
            <DashboardAnalytics
              ...
              setHeaderSubtitle={setPageSubtitleOverride}
            />
          ) : (
            <Outlet context={{ setHeaderTitle: setPageTitleOverride, setHeaderSubtitle: setPageSubtitleOverride }} />
          )}
        </main>
      </div>
    </div>
